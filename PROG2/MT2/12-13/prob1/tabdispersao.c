...
int tabela_adiciona(tabela_dispersao td, const objeto * valor)
{
    ...
    index = td->hfunc(valor->chave, td->tamanho);

    /* verifica se chave ja' existe na lista */
    elem = td->elementos[index];
    while (elem != NULL && strcmp(elem->obj->chave, valor->chave) != 0)
        elem = elem->proximo;
    ...
    if (elem == NULL)
    {
        ...
        /*copia chave e valor */
        strcpy(elem->obj->chave, valor->chave);
        strcpy(elem->obj->valor, valor->valor);
