/*****************************************************************/
/*          Biblioteca vetor | PROG2 | MIEEC | 2013/14           */      
/*****************************************************************/
...
int vetor_insere(vetor* vec, const char* valor, int pos)
{
	...
	/* se posicao for -1 insere no fim */
	if(pos == -1)
	{
		n = vec->tamanho;

		/* aloca espaco para a nova string no fim do vetor */
		vec->elementos[n].str = (char*)malloc((strlen(valor)+1)*sizeof(char));
		if(vec->elementos[n].str == NULL)
			return -1;

		/* copia valor */
		strcpy(vec->elementos[n].str,valor);
		vec->tamanho++;
